# -*- encoding: utf-8 -*-
# stub: fuubar 2.2.0 ruby lib

Gem::Specification.new do |s|
  s.name = "fuubar"
  s.version = "2.2.0"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.metadata = { "allowed_push_host" => "https://rubygems.org" } if s.respond_to? :metadata=
  s.require_paths = ["lib"]
  s.authors = ["Nicholas Evans", "Jeff Kreeftmeijer", "jfelchner"]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"]
  s.date = "2016-08-23"
  s.description = "the instafailing RSpec progress bar formatter"
  s.email = "[\"jeff@kreeftmeijer.nl\", \"accounts+git@thekompanee.com\"]"
  s.homepage = "https://github.com/thekompanee/fuubar"
  s.licenses = ["MIT"]
  s.rubygems_version = "2.5.1"
  s.summary = "the instafailing RSpec progress bar formatter"

  s.installed_by_version = "2.5.1" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<rspec-core>, ["~> 3.0"])
      s.add_runtime_dependency(%q<ruby-progressbar>, ["~> 1.4"])
      s.add_development_dependency(%q<rspec>, ["~> 3.0"])
      s.add_development_dependency(%q<chamber>, ["~> 2.3"])
      s.add_development_dependency(%q<awesome_print>, ["~> 1.7"])
    else
      s.add_dependency(%q<rspec-core>, ["~> 3.0"])
      s.add_dependency(%q<ruby-progressbar>, ["~> 1.4"])
      s.add_dependency(%q<rspec>, ["~> 3.0"])
      s.add_dependency(%q<chamber>, ["~> 2.3"])
      s.add_dependency(%q<awesome_print>, ["~> 1.7"])
    end
  else
    s.add_dependency(%q<rspec-core>, ["~> 3.0"])
    s.add_dependency(%q<ruby-progressbar>, ["~> 1.4"])
    s.add_dependency(%q<rspec>, ["~> 3.0"])
    s.add_dependency(%q<chamber>, ["~> 2.3"])
    s.add_dependency(%q<awesome_print>, ["~> 1.7"])
  end
end
